...
export class Vibi<S, P> {
  ...
  private adaptive_tolerance_ms(): number {
    ...
    const candidate = Math.round(ping / 2) + 20; // first guess buffer, 20 is typical small jitter
    const minTol = 80;
    const maxTol = this.tolerance; // treat provided tolerance as an upper bound
